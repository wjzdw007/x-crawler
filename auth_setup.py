        ...
        try:
            ...
            if (auth_info['cookies'].get('auth_token') and 
                ...
                if env_example.exists():
                    ...
                    # 替换认证值
                    env_updates = {
                        'X_AUTH_TOKEN': auth_info['cookies']['auth_token'],
                        'X_CT0_TOKEN': auth_info['cookies']['ct0'],
                        'X_CSRF_TOKEN': auth_info['headers'].get('x-csrf-token', auth_info['cookies']['ct0']),
                        'X_BEARER_TOKEN': auth_info['headers']['authorization']
                    }
                    ...
                    with open(env_file, 'w', encoding='utf-8') as f:
                        f.write(env_content)
                    saved_files.append('.env')
                    print("\n✅ 已保存到 .env 文件")

                # 2. 同时保存到 config.json（向后兼容）
                with open('config.json', 'w', encoding='utf-8') as f:
                    json.dump(config, f, ensure_ascii=False, indent=2)
                saved_files.append('config.json')

                print("\n🎉 认证配置完成!")
                print(f"💾 已保存到: {', '.join(saved_files)}")
                print("\n🚀 现在可以运行:")
